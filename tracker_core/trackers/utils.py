...
def asa_assignment(tracks, dets_high, dets_low, dets_del_high, match_thr, penalty_p, penalty_q,
                         reduce_step, frame_id, d_t=3):
    # Initialization
    matches = []
    dets = dets_high + dets_low + dets_del_high
    ...
    # =========================================================================
    # 2. 计算 ASA 补充项 (Supplement) - 用作奖励系数
    # =========================================================================
    ...
    # 参数设置：因为是作为修正项，我们可以放心使用大一点的参数来拉开区分度
    delta = 0.5
    lam = 0.75
    
    M, N = S.shape
    
    # 计算 Row Difference (Top1 - Top2)
    if N >= 2:
        sorted_rows = np.sort(S, axis=1)
        d_row = sorted_rows[:, -1] - sorted_rows[:, -2]
    else:
        d_row = np.zeros(M)
    d_row = np.minimum(d_row, delta)
    
    # 计算 Col Difference (Top1 - Top2)
    if M >= 2:
        sorted_cols = np.sort(S, axis=0)
        d_col = sorted_cols[-1, :] - sorted_cols[-2, :]
    else:
        d_col = np.zeros(N)
    d_col = np.minimum(d_col, delta)
    
    # 计算 ASA 权重 omega
    omega = (d_row[:, np.newaxis] + d_col[np.newaxis, :]) / 2.0 + lam
    
    # =========================================================================
    # 3. 融合 ASA (Fusion)
    # =========================================================================
    
    # 归一化 omega 到 0~1 之间 (最大可能值是 delta + lam)
    # omega_norm 越高，说明匹配越具有 "唯一性"
    omega_norm = omega / (delta + lam)
    
    # 设计奖励系数 (Correction Factor)
    # 逻辑：如果 S 高 (像) 且 omega_norm 高 (独特)，我们希望 Cost 变小
    # 0.5 是 "ASA影响力系数" (0.3~0.6 都是合理范围)
    # 举例：
    #  - 完美且独特匹配: S=1, omega=1 -> Factor = 1 - 0.5 = 0.5 (Cost 减半，极大奖励)
    #  - 模糊匹配: S=0.8, omega=0.2 -> Factor = 1 - 0.5*0.8*0.2 = 0.92 (Cost 几乎不变)
    
    asa_correction = 1.0 - (0.5 * omega_norm * S)
    
    # 应用修正
    cost = cost * asa_correction

    # =========================================================================
    # 4. 后续处理 (Penalty & Matching) - 保持不变
    # =========================================================================
